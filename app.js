...
function employeesHTML() {
 ...
 ${
    ...
    ? `
        <button
            class="secondary-btn"
            onclick="toggleEmployeeLogin(${employee.id})"
            style="margin-right:6px;"
        >
            Disable Login
        </button>
...
function hasC1PXPermission(permission) {
    ...
    if (window.currentEmployee.role === "Admin") return true;
...
window.saveEditedEmployee = async function(employeeId) {
    ...
    const role = document
        .getElementById("editEmployeeRole")
        .value;
...
window.createEmployeeLogin = async function (employeeId) {
    ...
    if (!currentEmployee || currentEmployee.role !== "Admin") {
        alert("Only Admin can create employee login accounts.");
        return;
    }
...
window.toggleEmployeeLogin = async function (employeeId) {
    ...
    if (!currentEmployee || currentEmployee.role !== "Admin") {
        alert("Only Admin can enable or disable employee login.");
        return;
    }
