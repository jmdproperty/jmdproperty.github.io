...
// C1PX login-seat limit: maximum 5 active employee logins per company,
// including the protected Owner account.
const C1PX_MAX_LOGIN_SEATS = 5;

function getActiveLoginSeatCount() {
    return companyEmployees.filter(e => e.login_enabled === true).length;
}

function hasAvailableLoginSeat() {
    return getActiveLoginSeatCount() < C1PX_MAX_LOGIN_SEATS;
}
...
function employeesHTML() {
    ...
    return `
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin:14px 0;">
            <div style="padding:10px 14px;border-radius:10px;background:#f8fafc;border:1px solid #e2e8f0;font-size:13px;font-weight:700;color:#334155;">
                Login seats: ${getActiveLoginSeatCount()}/${C1PX_MAX_LOGIN_SEATS}
                <span style="font-weight:500;color:#64748b;margin-left:6px;">(Owner counts as 1 seat)</span>
            </div>
            ${hasAvailableLoginSeat()
                ? `<span style="font-size:12px;color:#166534;">${C1PX_MAX_LOGIN_SEATS - getActiveLoginSeatCount()} login seat(s) available.</span>`
                : `<span style="font-size:12px;color:#b91c1c;font-weight:700;">Login limit reached. Disable or delete a login before creating another.</span>`}
        </div>
...
async function saveNewEmployee() {
    ...
    if (!isC1PXAdminOrOwner(window.currentEmployee)) {
        alert("Only Admin/Owner can add employees.");
        return;
    }
...
if (!currentCompany) {
    ...
    const { data, error } = await db.rpc("c1px_admin_create_employee", {
    ...
    if (error) {
        ...
        alert(`❌ Failed to save employee.\n\n${error.message || "Database rejected the employee."}`);
        return;
    }
...
window.editEmployee = function(employeeId) {
    ...
    if (!isC1PXAdminOrOwner(window.currentEmployee)) {
        alert("Only Admin/Owner can edit employees.");
        return;
    }
...
window.saveEditedEmployee = async function(employeeId) {
    ...
    // If this employee has a login and their email changed, update the Auth email
    // through the server-side Edge Function before saving the employee record.
    if (!protectedOwner && employee.auth_user_id && email !== (employee.email || "")) {
        try {
            const { data: emailResult, error: emailError } = await db.functions.invoke(
                "update-employee-email",
                { body: { employee_id: employeeId, email } }
            );
            if (emailError) throw new Error(emailError.message || "Failed to update login email.");
            if (!emailResult?.success) throw new Error(emailResult?.error || "Failed to update login email.");
        } catch (emailUpdateError) {
            console.error("Employee Auth email update error:", emailUpdateError);
            alert(emailUpdateError.message || "Failed to update login email.");
            return;
        }
    }

    const { data, error } = await db.rpc("c1px_admin_update_employee", {
    ...
    if (error) {
        ...
        alert(`❌ Failed to update employee.\n\n${error.message || "Database rejected the update."}`);
...
window.deleteEmployee = async function(employeeId) {
    ...
    // Remove the Supabase Auth login first (server-side).
    if (employee.auth_user_id) {
        try {
            const { data: authResult, error: authError } = await db.functions.invoke(
                "delete-employee-login",
                { body: { employee_id: employeeId } }
            );
            if (authError) throw new Error(authError.message || "Failed to delete employee login.");
            if (!authResult?.success) throw new Error(authResult?.error || "Failed to delete employee login.");
        } catch (deleteAuthError) {
            console.error("Employee Auth delete error:", deleteAuthError);
            alert(deleteAuthError.message || "Failed to delete employee login.");
            return;
        }
    }

    const { data: deleteResult, error } = await db.rpc("c1px_admin_delete_employee", {
        p_employee_id: employeeId
    });
    ...
    if (error) {
        ...
        alert(`❌ Failed to delete employee.\n\n${error.message || "Database rejected the deletion."}`);
...
function showLicenseScreen(message) {
    ...
    if (licenseMessage) licenseMessage.textContent = message || "CRM access is currently unavailable.";
...
async function checkC1PXLicense(showBlockedScreen = true) {
    ...
    if (error) {
        ...
        if (showBlockedScreen) showLicenseScreen("License service could not be verified. Please contact the administrator.");
...
function startC1PXLicenseWatch() {
    ...
    c1pxLicenseTimer = setInterval(async () => {
        ...
        if (!license?.allowed) {
            clearInterval(c1pxLicenseTimer);
            await db.auth.signOut();
            localStorage.removeItem("loggedInUser");
            sessionStorage.removeItem("c1pxSession");
...
async function initializeLoggedInUser(user) {
    ...
    // Check the company license BEFORE employee login_enabled.
    // A suspended company may have login_enabled=false as a hard lock,
    // but the user must see the renewal/lock screen instead of the generic
    // "employee login is disabled" message.
    ...
    // Only check the employee login switch after the company license is active.
    if (!employee.login_enabled) {
        await db.auth.signOut();
        throw new Error("Your employee login is disabled.");
    }
...
window.createEmployeeLogin = async function (employeeId) {
    ...
    if (!hasAvailableLoginSeat()) {
        alert(`Login limit reached. C1PX allows a maximum of ${C1PX_MAX_LOGIN_SEATS} active login accounts per company, including the Owner.`);
        return;
    }
...
window.toggleEmployeeLogin = async function (employeeId) {
    ...
    if (newStatus && !hasAvailableLoginSeat()) {
        alert(`Login limit reached. C1PX allows a maximum of ${C1PX_MAX_LOGIN_SEATS} active login accounts per company, including the Owner.`);
        return;
    }
